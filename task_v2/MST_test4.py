...
from MST_train4 import AdvancedStemUNet, custom_collate_fn_ignore_none
...
# --- 実験９ ---
def evaluate_and_visualize(beta=50,W_chamfer=W_chamfer, W_smooth=1.0, DATA=DATA,test_num=1,num_points=10):
    unet_backbone = AdvancedStemUNet(in_channels=4, out_channels=num_points - 2)
    ...
    MODEL_WEIGHTS_PATH = f"MST_model4_t_gt{test_num}.pth"
    SAVE_DIR = f"MST_model4_t_gt{test_num}"
    SAVE_HIST_DIR = f"MST_hist4_t_gt{test_num}"
    SAVE_CSV_DIR = f"MST_csv4_t_gt{test_num}"
    SAVE_DIR_TRAIN = f"MST_check4_{test_num}_{DATA}"
    ...
    if DATA == "train":
        test_dataset = TomatoStemDataset(PT_SPLIT_TRAIN_PATH, COLOR_PATH, GT_POINTCLOUD_PATH, num_control_points=num_points, augment=False)
    else:
        test_dataset = TomatoStemDataset(PT_SPLIT_TEST_PATH, COLOR_PATH, GT_POINTCLOUD_PATH, num_control_points=num_points, augment=False)
    ...
    with torch.no_grad():
        for batch in tqdm(test_loader):
            ...
            plt.scatter(sorted_pred_ctrl_px[:, 0], sorted_pred_ctrl_px[:, 1], color='cyan', s=100, zorder=5, edgecolors='black', label=f'Control Points (N={num_points})')
    ...
    if count > 0:
        ...
        # binsを20に設定し、IoU 0.0〜1.0 の範囲で分布を描画
...
if __name__ == "__main__":
    avg_iou, avg_cd = evaluate_and_visualize(DATA="test", test_num=1,beta=1.0,num_points=15)
    avg_iou, avg_cd = evaluate_and_visualize(DATA="test", test_num=2,beta=1.0,num_points=15)
    avg_iou, avg_cd = evaluate_and_visualize(DATA="test", test_num=3,beta=1.0,num_points=15)
