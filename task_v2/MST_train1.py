...
def simple_hybrid_loss(pred_curve, gt_curve, growth_points, w_chamfer=3.0, w_l1=1.0):
    ...
    GTの向きを成長点スタートに揃えてから、ChamferとL1を計算します。
    """
    B = pred_curve.size(0)
    aligned_gts = []
...
# def curve_loss(pred_curve, gt_curve, control_points, w_chamfer=10.0, w_uniform=1.0):
#     """
#     L1 Lossを完全に排除し、Chamfer(形状) + Uniformity(等間隔性) で学習する新しいLoss
#     """
#     B = pred_curve.size(0)
    
#     # ① L2 Chamfer (L1が無いため、GTの向きを揃える処理は不要になりました)
...
#     # 総合Loss
#     return 100.0 * (w_chamfer * chamfer_loss + w_uniform * uniformity_loss)
...
def structured_heatmap_loss(pred_logits, gt_points, epsilon=1.0, alpha=1.0):
    ...
    # 警告を消すために、明示的に両方のテンソルを [B, K, H, W, 2] のサイズに引き延ばす
    ...
    # ※ pred_logits が論文の F(y_hat) に相当します
...
def train_and_evaluate(l_r, pa, number, train_files, val_files, W_chamfer, W_smooth, W_uniform, test_num, beta=50, W_heatmap=1000.0):
    ...
    model_save_name = f"MST_model1_t_gt{test_num}.pth"
...
if __name__ == "__main__":
    ...
    train_and_evaluate(
                ...
                val_files=val_files, 
                W_chamfer=10.0, 
                W_smooth=2.0, # 変数名を合わせる
                W_uniform=2.0, # 変数名を合わせる
                test_num=10,
                beta=1.0
            )
