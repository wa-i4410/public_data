...
# ★ Transformerモデルをインポート
from MST_model2 import TomatoStemDataset, StemTransformerModel
from MST_train2 import custom_collate_fn_ignore_none
...
def calculate_pixel_chamfer_distance(pred_pts, gt_pts):
    ...
    return (np.mean(min_dist_pred_to_gt) + np.mean(min_dist_gt_to_pred)) / 2.0
...
# ===== メイン推論・可視化処理 =====
def evaluate_transformer(w_l1=100.0, w_chamfer=10.0, w_smooth=1.0):
    model = StemTransformerModel(num_queries=CONTROL_POINTS - 2).to(device)

    MODEL_WEIGHTS_PATH = f"MST_model2_try10_TF_l1_{w_l1}_c{w_chamfer}_s{w_smooth}.pth"
    SAVE_DIR = f"MST_model2_try10_TF_results"
    SAVE_HIST_DIR = f"MST_model2_try10_TF_hist"
    SAVE_CSV_DIR = f"MST_model2_try10_TF_csv"
    ...
    with torch.no_grad():
        for batch in tqdm(test_loader):
            ...
            # ★ Transformerは正しい順序で出力するので、並べ替え(sort)は行わずそのまま使う！
            sorted_pred_ctrl_px = pred_ctrl_px
    ...
    if count > 0:
        csv_filename = os.path.join(SAVE_CSV_DIR, f"evaluation_results_TF.csv")
        with open(csv_filename, mode='w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            writer.writerow(['File Name', 'IoU', 'Chamfer Distance (px)'])
            writer.writerows(results_data)
        
        plt.figure(figsize=(10, 6))
        plt.hist(iou_list, bins=20, range=(0.0, 1.0), color='skyblue', edgecolor='black')
        plt.title(f'IoU Distribution (Transformer)')
        plt.xlabel('IoU')
        plt.ylabel('Frequency (Number of Images)')
        plt.grid(axis='y', alpha=0.75)
        
        hist_filename = os.path.join(SAVE_HIST_DIR, f"iou_histogram_TF.png")
        plt.savefig(hist_filename, bbox_inches='tight', dpi=150)
        plt.close()

        print("\n" + "="*40)
        print(f"📝 評価完了! (テストデータ {count} 件)")
        print(f"   Average Line IoU    : {total_iou / count:.4f}")
        print(f"   Average Chamfer Dist: {total_cd / count:.2f} px")
        print("="*40)

if __name__ == "__main__":
    evaluate_transformer(w_l1=100.0, w_chamfer=10.0, w_smooth=1.0)