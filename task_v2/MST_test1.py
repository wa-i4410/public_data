...
from MST_model1 import TomatoStemDataset, StemCurveModel
...
# --- 実験９ ---
def evaluate_and_visualize(beta=50,W_chamfer=W_chamfer, W_smooth=1.0, DATA=DATA,test_num=1):
    ...
    # 実験６
    # MODEL_WEIGHTS_PATH = f"MST_model1_try5_c{W_chamfer}.pth"
    # SAVE_DIR = f"MST_model1_try6_c{W_chamfer}"
    # SAVE_HIST_DIR = f"MST_histgram6_u{W_chamfer}"
    # SAVE_CSV_DIR = f"MST_csv6_u{W_chamfer}"
    # SAVE_DIR_TRAIN = f"MST_check_{DATA}"

    # 実験７
    # MODEL_WEIGHTS_PATH = f"MST_model1_try7_s{W_smooth}.pth"
    # SAVE_DIR = f"MST_model1_try7_s{W_smooth}"
    # SAVE_DIR_TRAIN = f"MST_check_{DATA}"

    # 実験８
    # MODEL_WEIGHTS_PATH = f"MST_model1_try8_c10.0_s1.0_u{W_uniform}.pth"
    # SAVE_DIR = f"MST_model1_try8_u{W_uniform}"
    # SAVE_HIST_DIR = f"MST_histgram8_u{W_uniform}"
    # SAVE_CSV_DIR = f"MST_csv8_u{W_uniform}"
    # SAVE_DIR_TRAIN = f"MST_check_{DATA}"  

    # 実験９
    # MODEL_WEIGHTS_PATH = f"MST_model1_try9_b{beta}_c{W_chamfer}_s{W_smooth}.pth"
    # SAVE_DIR = f"MST_model1_try9_b{beta}"
    # SAVE_HIST_DIR = f"MST_histgram9_b{beta}"
    # SAVE_CSV_DIR = f"MST_csv9_b{beta}"
    # SAVE_DIR_TRAIN = f"MST_check_{DATA}"

    
    MODEL_WEIGHTS_PATH = f"MST_model1_t_gt{test_num}.pth"
    SAVE_DIR = f"MST_model1_t_gt{test_num}"
    SAVE_HIST_DIR = f"MST_hist1_t_gt{test_num}"
    SAVE_CSV_DIR = f"MST_csv1_t_gt{test_num}"
    SAVE_DIR_TRAIN = f"MST_check_{DATA}"
...
if __name__ == "__main__":
    # # --- 実験６ ---
    # w_list = [3.0,5.0,10.0]  # 必要に応じて複数の重みでループ
    # for W_chamfer in w_list:  # 必要に応じて複数の重みでループ
    #     evaluate_and_visualize(W_chamfer=W_chamfer, DATA="test")  # テストデータで評価
    # evaluate_and_visualize(W_chamfer=10.0, DATA="test")  # テストデータで評価
    

    # --- 実験７ ---
    # s_list = [1.5,2.0,3.0]  # 必要に応じて複数の重みでループ
    # for W_smooth in s_list:  # 必要に応じて複数の重みでループ
    #     evaluate_and_visualize(W_smooth=W_smooth, DATA="test")  # テストデータで評価
    
    # # --- 実験８ ---
    # u_list = [0.0, 0.1]  # 必要に応じて複数の重みでループ
    # for W_uniform in u_list:  # 必要に応じて複数の重みでループ
    #     evaluate_and_visualize(W_uniform=W_uniform, DATA="test")  # テストデータで評価        
     
    # # --- 実験９ ---   
    # beta_list = [10.0, 50.0]  # 必要に応じて複数の重みでループ
    # for beta in beta_list:  # 必要に応じて複数の重みでループ
    #     evaluate_and_visualize(beta=beta, W_chamfer=10.0, W_smooth=1.0, DATA="test")  

    # --- 対照実験 ---
    # summary_results = []  # ★ 追加: 各実験の平均結果を保存するリスト

    # for num in range(6):
    #     test_num = num + 1
    #     print(f"\n🚀 実験 {test_num} の評価を開始します...")
        
    #     # ★ 変更: 関数から平均IoUと平均Chamferを受け取る
    #     avg_iou, avg_cd = evaluate_and_visualize(DATA="test", test_num=test_num) 
        
    #     # リストに「実験番号、平均IoU、平均Chamfer距離」を追加
    #     summary_results.append([f"Experiment_{test_num}", avg_iou, avg_cd])

    # # --- ★ 追加: 12回分の結果を1つのCSVにまとめて保存 ---
    # summary_csv_path = "summary_evaluation_results_all.csv"
    # with open(summary_csv_path, mode='w', newline='', encoding='utf-8') as f:
    #     writer = csv.writer(f)
    #     writer.writerow(['Experiment', 'Average IoU', 'Average Chamfer Distance (px)'])
    #     writer.writerows(summary_results)
        
    # print("\n" + "🌟"*20)
    # print(f"🎉 対照実験 6回分の全サマリーを '{summary_csv_path}' に保存しました！")
    # print("🌟"*20)

    avg_iou, avg_cd = evaluate_and_visualize(DATA="test", test_num=10,beta=1.0)
    avg_iou, avg_cd = evaluate_and_visualize(DATA="test", test_num=11,beta=1.0)
    avg_iou, avg_cd = evaluate_and_visualize(DATA="test", test_num=12,beta=1.0)
