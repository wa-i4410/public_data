...
from newmodel import UNet, UNetDataset 
...
def train_unet(l_r, NUM_EPOCHS, sig, fine_tune_from=None, sigma_decay_epochs=50):
    ...
    for epoch in range(epochs):
        ...
        if (epoch + 1) % sigma_decay_epochs == 0 and alternative > 0.1: # 0.1より小さくならないように
        ...
        if avg_test_loss < best_test_loss:
            ...
            # ★ 修正点: 保存するファイル名に、現在のsigma値(sig)と、
            # ファインチューニング元(fine_tune_from)の情報を入れると管理しやすい
